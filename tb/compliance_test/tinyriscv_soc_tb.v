...
// ============================================================
// I2C V2 Slave - receive Addr+R then send 2 data bytes (no ACK)
// ============================================================
...
always @(posedge clk or posedge rst) begin
    if (rst == `RstEnable) begin
        ...
        case (vs)
            ...
            // ACK: hold SDA low, then start DATA0 on next SCL fall
            V2_SACK: begin
                sda_oe <= 1; sda_o <= 0;
                if (v2_sfe) begin
                    vs <= V2_SEND0; vb <= 0;
            ...
            // wait master ACK, then send second byte
            V2_MACK: begin
                if (v2_sfe) begin
                    vs <= V2_SEND1; vb <= 0;
