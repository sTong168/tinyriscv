...
module uart_debug(
    ...
    output wire mem_valid_o  //合并新增部分(1/2)
...
//合并新增部分(2/2)
    ...
    assign mem_access_valid =
        (mem_we_o &&
         state != S_SEND_ACK &&
         state != S_SEND_NAK &&
         !(state == S_WRITE_MEM &&
           write_mem_byte_index0 == (need_to_rec_bytes + 8'd2))) ||
        state == S_WAIT_BYTE2 ||
        state == S_GET_BYTE;

    assign mem_valid_o = req_o && mem_access_valid;
    ...
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    // gxy新增部分1/2
                    if (ack_i == 1'b1 && write_mem_byte_index0 != (need_to_rec_bytes + 2)) begin
