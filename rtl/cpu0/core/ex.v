...
module ex(
    ...
    // from i2c_send
    input wire i2c_busy_i,                 // I2C发送忙标志
    input wire[`MemBus] i2c_wdata_i,       // I2C发送写数据
    ...
    assign reg_wdata_o = reg_wdata | i2c_reg_wdata_i | if_reg_wdata_i;
    assign reg_we_o = reg_we || i2c_reg_we_i || if_reg_we_i;
    assign reg_waddr_o = reg_waddr | i2c_reg_waddr_i | if_reg_waddr_i;

    assign mem_we_o = mem_we;
    assign mem_req_o = mem_req;

    assign hold_flag_o = hold_flag || uart_hold_flag || i2c_hold_flag || if_hold_flag;
    assign jump_flag_o = jump_flag;
    assign jump_addr_o = jump_addr | uart_jump_addr | i2c_jump_addr | if_jump_addr;
    ...
    always @ (*) begin
        if (i2c_busy_i == `True) begin
            i2c_hold_flag = `HoldEnable;
            i2c_jump_addr = inst_addr_i;
        end else begin
            i2c_hold_flag = `HoldDisable;
            i2c_jump_addr = `ZeroWord;
        end
    end
